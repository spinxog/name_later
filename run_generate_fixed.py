...
def generate_synthetic_dataset(image_dir, output_dir, num_samples=50000, seed=42):
    ...
    for idx in tqdm(range(num_samples), desc="Generating synthetic samples"):
        ...
        patch_y = random.randint(0, max(0, patch_img.shape[0] - patch_h))
        patch_x = random.randint(0, max(0, patch_img.shape[1] - patch_w))
        patch = patch_img[patch_y:patch_y+patch_h, patch_x:patch_x+patch_w]

        paste_y = random.randint(0, max(0, image.shape[0] - patch_h))
        paste_x = random.randint(0, max(0, image.shape[1] - patch_w))
