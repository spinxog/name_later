...
def generate_synthetic_dataset(image_dir, output_dir, num_samples=50000, seed=42):
    ...
    dataset = ForgeryDataset(image_dir, synthetic=True)  # Dummy to access method

    for idx in tqdm(range(num_samples), desc="Generating synthetic samples"):
        # Generate sample
        image, mask = dataset._generate_synthetic_sample()

        # Convert back to numpy for saving (since transform is applied)
        # Wait, in _generate_synthetic_sample, if transform is applied, image is tensor, mask is tensor
        # But we need to save as image and mask numpy
        # So, modify to return numpy before transform

        # Actually, let's modify the method to return numpy arrays without transform for saving
        # Or save after transform? But for pretrain, probably save raw or transformed?

        # To keep it simple, generate without transform, save, then apply transform during loading.

        # But current _generate_synthetic_sample applies transform if self.transform is set.

        # For generation, set transform=None temporarily.

        # Better to duplicate the logic without transform.
...
if __name__ == "__main__":
    # Assuming image_dir is 'data/train_images' or similar
    # Adjust paths as needed
    image_dir = 'data/train_images'  # Adjust to actual path
